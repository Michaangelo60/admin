// Resolve API base at runtime and avoid clashing with other pages that may
// declare the same global. Use a page-local constant `ADMIN_API_BASE`.
const ADMIN_API_BASE = (window.API_BASE || window.API_URL || location.origin).replace(/\/$/, '');

console.debug('transaction-admin-site: script.js loaded, ADMIN_API_BASE=', ADMIN_API_BASE);
...
if (document.getElementById('tx-list')) {
  ...
  const pendingBtn = qs('#pending-btn');
  ...
  if (pendingBtn) pendingBtn.addEventListener('click', () => loadTransactions('pending'));
  ...
  async function promoteToAdmin() {
    ...
    try {
      ...
      const res = await fetch(ADMIN_API_BASE + '/api/dev/promote', { method: 'POST', headers: { 'Authorization': 'Bearer ' + token } });
      const j = await res.json().catch(()=>({}));
      if (!res.ok) {
  ...
  async function loadTransactions(status) {
    ...
    try {
      const token = localStorage.getItem('token') || '';
      const url = ADMIN_API_BASE + '/api/transactions' + (status ? ('?status=' + encodeURIComponent(status)) : '');
  ...
  function renderList(list, isAdmin) {
    ...
    txListEl.innerHTML = list.map(t => {
      ...
      const txId = t._id || t.id || t.transactionId || '';
      const statusText = t.status ? String(t.status) : 'unknown';
      let controlsHtml = '';
      if (isAdmin) {
        // Allow admins to confirm both 'pending' and 'processing' transactions
        if (['pending','processing'].includes(String(statusText).toLowerCase())) {
    ...
    if (isAdmin) {
      qsa('.confirm-btn').forEach(btn => {
        btn.addEventListener('click', async (e) => {
          ...
          try {
            const token = localStorage.getItem('token') || '';
            const res = await fetch(ADMIN_API_BASE + '/api/transactions/' + id + '/status', {
            ...
            try { await fetch(ADMIN_API_BASE + '/api/auth/me', { headers: { 'Authorization': 'Bearer ' + token } }).then(r=>r.json()).then(j=>{ if (j && j.data) localStorage.setItem('user', JSON.stringify(j.data)); }); } catch(e){}
          } catch (err) {
